...
describe("FilesystemObjectStore", () => {
  it("atomically round-trips bytes and returns a portable integrity manifest", async () => {
    const root = await temporaryDirectory("monte-carlo-store-");
    ...
    expect(
      workspaceFiles.some((name) => name.includes(".monte-carlo-") && name.endsWith(".tmp")),
    ).toBe(false);
  });

  it("rejects traversal and symlinked workspace paths", async () => {
    const root = await temporaryDirectory("monte-carlo-store-");
    const outside = await temporaryDirectory("monte-carlo-outside-");
